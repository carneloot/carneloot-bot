...
const buildBot = () => {
	...
	const setWebhook = (url: string) => bot.api.setWebhook(url);
...
export class BotService extends Effect.Service<BotService>()('app/BotService', {
	effect: Effect.sync(() => {
		const { bot, setCommands, setWebhook } = buildBot();
