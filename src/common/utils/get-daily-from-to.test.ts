...
describe('getDailyFromTo', () => {
	...
	it('returns from and to dates when now is before dayStart', () => {
		...
		expect(result.from).toEqual(
			zonedTimeToUtc(new Date(2024, 2, 4, 4, 0, 0), timezone)
		);
		expect(result.to).toEqual(
			zonedTimeToUtc(new Date(2024, 2, 5, 4, 0, 0), timezone)
		);
	});
	...
	it('returns from and to dates when now is after dayStart', () => {
		...
		const result = getDailyFromTo(now, dayStart);
		expect(result.from).toEqual(
			zonedTimeToUtc(new Date(2024, 2, 7, 7, 0, 0), timezone)
		);
		expect(result.to).toEqual(
			zonedTimeToUtc(new Date(2024, 2, 8, 7, 0, 0), timezone)
		);
	});
	...
	it('returns from and to dates when now hour is exactly at dayStart', () => {
		...
		expect(result.from).toEqual(
			zonedTimeToUtc(new Date(2024, 2, 7, 9, 0, 0), timezone)
		);
		expect(result.to).toEqual(
			zonedTimeToUtc(new Date(2024, 2, 8, 9, 0, 0), timezone)
		);
	});
});
